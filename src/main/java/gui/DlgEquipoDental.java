...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void listar() {
		...
		try {
			...
			for (EquipoDental equipoDental : lstEquipoDental) {
				...
				imprimir("Fecha de adquisición.: " + equipoDental.getFechaAdquisicion());
				imprimir("Dentista.............: " + dentista.getNombreCompleto());
				imprimir("Correo...............: " + dentista.getCorreo());
				imprimir("Especialidad.........: " + dentista.getEspecialidad());
	...
	void adicionar() {
		...
		try {
			EquipoDental equipoDental = new EquipoDental(null, nombre, costo, fecha, estado, dentista);
	...
	void buscar() {
		...
		try {
			EquipoDental equipoDental = manager.find(EquipoDental.class, nroEquipo);
			...
			txtFechaAdquisicion.setText(equipoDental.getFechaAdquisicion()+"");
	...
	void modificar() {
		...
		Integer nro = Integer.parseInt(txtNroEquipo.getText());
		String nombre = txtNombre.getText();
		Double costo = Double.parseDouble(txtCosto.getText());
		String estado = cboEstados.getSelectedItem().toString();
		LocalDate fecha = LocalDate.parse(txtFechaAdquisicion.getText());
		Dentista dentista = (Dentista) cboDentistas.getSelectedItem();
		
		EntityManager manager = JPAUtil.getEntityManager();
		
		try {
			EquipoDental equipoDental = new EquipoDental(nro, nombre, costo, fecha, estado, dentista);
